import { getSession } from '@/lib/auth';
import { redirect } from 'next/navigation';
import { resolveTenantContext } from '@/lib/tenant-context.server';
import { tenantFeatureEnabled } from '@/lib/feature-flags';
import { MODULE_ROLE_ACCESS } from '@/lib/constants/modules-registry';
import { listDeliveryDriversAction } from '@/app/actions/delivery.actions';
import { MotorizadosView } from './motorizados-view';

export const dynamic = 'force-dynamic';

export const metadata = {
    title: 'Motorizados | KPSULA',
    description: 'Gestión de motorizados de delivery',
};

const ALLOWED_ROLES = MODULE_ROLE_ACCESS['delivery'] ?? [];

export default async function MotorizadosPage() {
    const session = await getSession();
    if (!session) redirect('/login');
    if (!ALLOWED_ROLES.includes(session.role)) redirect('/dashboard');

    let enabled = false;
    try {
        const { tenantId } = await resolveTenantContext();
        enabled = await tenantFeatureEnabled(tenantId, 'deliveryOps');
    } catch {
        enabled = false;
    }
    if (!enabled) redirect('/dashboard');

    const { drivers, branches } = await listDeliveryDriversAction();

    return <MotorizadosView initialDrivers={drivers} branches={branches} />;
}
