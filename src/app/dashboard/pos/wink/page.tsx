...
import { Search, X as XIcon, ShoppingBag, Truck, Printer } from 'lucide-react';
import { useUIStore } from '@/stores/ui.store';
import { getMenuForPOSAction, type CartItem } from '@/app/actions/pos.actions';
import { createWinkOrderAction } from '@/app/actions/wink.actions';
...
export default function POSWinkPage() {
    ...
    // Precio WINK: override manual si existe; si no, el precio base del producto.
    const getWinkPrice = (item: MenuItem) => item.winkPrice ?? item.price;
    const hasWinkOverride = (item: MenuItem) => item.winkPrice != null && item.winkPrice !== item.price;
    ...
    const handleSubmit = async () => {
        ...
        try {
            const result = await createWinkOrderAction({
    ...
    if (isLoading) return (
        <div className="flex min-h-screen items-center justify-center bg-capsula-ivory">
            <div className="flex flex-col items-center gap-3 text-center">
                <Truck className="h-10 w-10 text-capsula-coral" />
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="font-semibold text-lg tracking-[-0.02em] text-capsula-ink md:text-2xl">
                            POS <span className="text-capsula-coral">WINK</span>
                        </h1>
                                ...
                                >
                                    ...
                                    <div>
                                        <div className="font-semibold text-2xl tabular-nums tracking-[-0.02em] text-capsula-coral">
                                            ${getWinkPrice(item).toFixed(2)}
                                        </div>
                                        {hasWinkOverride(item) && (
                ...
                <div className="z-20 flex w-80 flex-col border-l border-capsula-line bg-capsula-ivory-surface shadow-cap-soft tablet-land:w-96 xl:w-96">
                    ...
                    <div className="space-y-3 border-t border-capsula-line bg-capsula-ivory-surface p-4">
                        ...
                        <div className="rounded-xl border border-capsula-coral/30 bg-capsula-coral-subtle px-3 py-2 text-xs text-capsula-coral">
                            <strong>WINK gestiona el cobro.</strong> Este registro es solo para inventario y cocina. No se genera cobranza interna.
                        </div>
                        <button
                        ...
                        <div className="flex justify-between border-b border-capsula-line p-5">
                            <div>
                                ...
                                <p className="font-semibold text-xl tabular-nums tracking-[-0.02em] text-capsula-coral">
                                    ${getWinkPrice(selectedItemForModifier).toFixed(2)}
                                    {hasWinkOverride(selectedItemForModifier) && (
