...
export default function POSDeliveryPage() {
    ...
    const confirmAddToCart = () => {
        ...
        const explodedModifiers = currentModifiers.flatMap(m => Array(m.quantity).fill({ modifierId: m.id, name: m.name, priceAdjustment: m.priceAdjustment, hideFromKitchen: parentModIds.has(m.id) }));
        ...
        try {
            ...
            if (result.success && result.data) {
                ...
                const receiptData = {
                    ...
                    deliveryFeeLabel: deliveryFeeMode === 'DIVISAS' ? 'Dólares' : 'Bs',
                    total: finalTotal,
                            ...
                            <div className="space-y-2 rounded-2xl border border-capsula-line bg-capsula-ivory-surface p-3">
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                    <button
                                        ...
                                        className={`inline-flex items-center justify-center gap-1.5 rounded-xl border px-2 py-3 text-sm font-semibold transition-colors active:scale-95 ${
                                            deliveryFeeMode === 'DIVISAS'
                                                ? 'border-[#2F6B4E] bg-[#2F6B4E] text-white'
                                                : 'border-capsula-line bg-capsula-ivory text-[#2F6B4E] hover:bg-[#E5EDE7]/60'
                                        }`}
