...
## §46 🚨 BUG TAB-2433 — propina fantasma con descuentos + mesero (pendiente fix 2026-06-06 AM)

**Estado**: documentado, NO corregido. Acordado con el dueño: el fix se aplica
mañana en la mañana con el sitio cerrado para poder tolerar el redeploy. La
producción sigue operativa con la mitigación temporal de abajo.
...
### Plan de fix (a ejecutar 2026-06-06 AM con sitio cerrado)

**Paso 1 — Fix Bug A (1 línea de código)**
- `src/app/actions/pos.actions.ts:2095`: cambiar `openTab.runningSubtotal`
  por `openTab.runningTotal`.
- `prisma/schema.prisma`: actualizar el comentario de `OpenTab.tipAmount` a
  `tipPercent/100 × runningTotal` para que docs y código cuadren.

**Paso 2 — Fix Bug B (guard de validación)**
- En `handlePaymentPinConfirm` (`restaurante/page.tsx:1112`), antes de
  `recordCollectiveTipAction`, validar que el monto efectivamente entregado
  por el cliente cubre `total + tipVal`. Si no, NO crear la propina colectiva
  (cap a `max(0, amountReceived - total)`).
- Alternativa más conservadora: mostrar el modal de confirmación de pago
  existente (`PaymentConfirmationModal`) con la propina prominente y obligar
  a la cajera a confirmar.

**Paso 3 — Tests nuevos (`src/lib/pos/tip-calculation.test.ts`)**
- 4 casos: sin descuento, con DIVISAS_33, con CORTESIA_PERCENT, con
  CORTESIA_100. Validar que `tipAmount = tipPercent/100 × runningTotal` en
  todos.
- Más test del guard de Bug B: propina creada nunca excede el excedente real.
